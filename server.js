...
io.sockets.on('connection', function(socket) {
    ...
    // Execute the Python script as a child process
    exec(`python generate_music_list.py --hours ${jsonData.hours} --genre "${jsonData.genre}" --mood "${jsonData.mood}" --token "${jsonData.access_token}"`, (error, stdout, stderr) => {
      if (error) {
        console.error(`Error executing Python script: ${error.message}`);
      ...
      // Emit the script response to the client
      socket.emit('get_music_list_response', {status: 1, list_data: parsePlaylist(stdout)})
    })
    });
    ...
    // Execute the Python script as a child process
    exec(`python create_playlist.py --token "${jsonData.access_token}" --music_array "${jsonData.musicArray}"`, (error, stdout, stderr) => {
      if (error) {
        console.error(`Error executing Python script: ${error.message}`);
